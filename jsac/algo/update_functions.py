...
def critic_update(rng, 
    ...
    if resnet_model:
        batch_stats = critic.batch_stats
    else:
        batch_stats = {}
        
    # print('\n\n________++_______\n\n')
    # print(jax.tree_util.tree_map(lambda x: x.shape, batch_stats))
    # print('\n\n________++_______\n\n')

        
    _, next_actions, next_log_probs, _ = actor.apply_fn(
        {"params": actor.params, 'batch_stats': batch_stats}, 
        keys_ac,
        batch.next_images, 
    ...
    target_Q1, target_Q2 = critic_target.apply_fn(
        {"params": critic_target.params, 'batch_stats': batch_stats}, 
        keys_crt,
        batch.next_images, 
        batch.next_proprioceptions, 
        next_actions,
    ...
    def critic_loss_fn_resnet(critic_params):
        (q1, q2), updates = critic.apply_fn( 
    ...
    if resnet_model:
        grads, info = jax.grad(critic_loss_fn_resnet, has_aux=True)(critic.params) 
        critic_new = critic.apply_gradients(grads=grads, batch_stats=info['updates']['batch_stats'])
        info = {'critic_loss': info['critic_loss'], 'q1': info['q1'], 'q2': info['q2']}
    else:
...
def actor_update(rng, 
    ...
    def actor_loss_fn(actor_params):    
        ...
        q1, q2 = critic.apply_fn(
            {'params': critic.params, 'batch_stats': batch_stats}, 
            keys_cr,
            batch.images, 
            batch.proprioceptions, 
            actions, 
